...
def user_just_switched_into_zoroark(battle, switch_or_drag):
    ...
    return (
        ...
        (
            ...
            and battle.user.last_selected_move is not None
            and battle.user.last_selected_move.move
...
def check_speed_ranges(battle, msg_lines):
    ...
    if (
        ...
        user_move_name = normalize_name(battle.user.last_selected_move.move) if battle.user.last_selected_move and battle.user.last_selected_move.move else ""
        if user_move_name and user_move_name in all_move_json:
            moves.append(
                (
                    "{}a: {}".format(battle.opponent.name, battle.user.active.name),
                    all_move_json[user_move_name],
                )
            )
        else:
            return
...
def check_choicescarf(battle, msg_lines):
    ...
    if any(
        ...
        for ln in msg_lines
    ) or not battle.user.last_selected_move or not battle.user.last_selected_move.move or battle.user.last_selected_move.move.startswith("switch "):
        return
    ...
    if number_of_moves not in [1, 2] or moves[0][0].startswith(battle.user.name):
        ...
        user_move_name = normalize_name(battle.user.last_selected_move.move)
        if not user_move_name or user_move_name not in all_move_json:
            return
        moves.append(
            (
                "{}a: {}".format(battle.opponent.name, battle.user.active.name),
...
def _update_multi_active(battle, msg):
    """Update handler for multi-active (doubles/triples) formats under Gemini.

    Accumulates msg_list for Gemini's msg_parser to read, updates request_json
    and control flags, but skips singles-only process_battle_updates.
    """
    msg_lines = msg.split("\n")
    for line in msg_lines:
        split_msg = line.split("|")
        ...
        if action == "request":
            request(battle, split_msg)
            # Do NOT call process_battle_updates — Gemini reads msg_list directly
            return not battle.wait
        else:
            battle.msg_list.append(line)

            # Capture server errors for Gemini retry
            if action == "error":
                error_text = "|".join(split_msg[2:]).strip() if len(split_msg) > 2 else "Unknown error"
                battle.last_server_error = error_text
                logger.warning("Server error captured for Gemini retry: %s", error_text)
...
async def async_update_battle(battle, msg):
    ...
    # Capture |error| messages regardless of engine
    if "|error|" in msg:
        for line in msg.split("\n"):
            parts = line.split("|")
            if len(parts) >= 3 and parts[1].strip() == "error":
                battle.last_server_error = "|".join(parts[2:]).strip()

    # Route to multi-active handler for Gemini with non-singles formats
    if (
        FoulPlayConfig.decision_engine == "gemini"
        and battle.format_info is not None
        and battle.format_info.gametype != "singles"
    ):
        result = _update_multi_active(battle, msg)
    else:
        result = update_battle(battle, msg)

    if getattr(battle, "last_server_error", None):
        return True
