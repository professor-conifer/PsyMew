"""TutorSession — educational coaching mode powered by DeepSeek.

Provides post-turn commentary, responds to opponent chat, and gives
a post-game review. Uses persistent message history via the OpenAI SDK.
...
from fp.deepseek.client import get_client
...
class TutorSession:
    """Persistent DeepSeek chat session for tutoring/coaching during battle."""
    ...
    def _get_client(self):
        if self._client is None:
            ...
            self._client = get_client(
                api_key_override=FoulPlayConfig.deepseek_api_key,
            )
            self._model = getattr(
                FoulPlayConfig, "deepseek_tutor_model", "deepseek-v4-flash"
            )
        return self._client
    ...
    async def _ask(self, prompt: str, system_context: str = "") -> Optional[str]:
        """Send a prompt to DeepSeek and return a single sanitized reply string."""
        ...
        for attempt in range(2):
            try:
                ...
                if any(
                    ...
                    wait = 3 * (attempt + 1)
                    logger.warning(
                        "Tutor %s (attempt %d/2), retrying in %ds...",
                        err_str[:80],
                        attempt + 1,
                        wait,
                    )
    ...
    async def on_battle_start(
        self, format_name: str, format_rules_text: str
    ) -> Optional[str]:
        ...
        return await self._ask(
            prompt, system_context=f"Format: {format_rules_text[:150]}"
        )
    ...
    async def on_battle_end(
        self, winner: Optional[str], bot_username: str
    ) -> Optional[str]:
