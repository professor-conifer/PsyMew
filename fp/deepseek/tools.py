"""Build OpenAI function-calling tool schemas for DeepSeek.

Converts the same view-aware distribution logic used by Gemini and
Claude into OpenAI's {type: "function", function: {name, description,
parameters}} format.

Rebuilt fresh each turn so enum values always reflect the current
battle state.
"""
...
def _build_distribution_tool(view: GeminiBattleView) -> dict:
    """Build the distribution-style action selection tool in OpenAI format."""
...
def _build_team_preview_tool(view: GeminiBattleView) -> dict:
    """Build the team preview lead-selection tool in OpenAI format."""
...
def build_tools(view: GeminiBattleView) -> list[dict]:
    """Build OpenAI function-calling tool declarations for this turn.
    ...
    -------
    list[dict]
        Tool declarations in OpenAI format
