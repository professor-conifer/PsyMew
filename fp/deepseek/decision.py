"""DeepSeek decision engine — main entry point for move selection.

find_best_move_deepseek() builds a GeminiBattleView, runs MCTS search
in parallel for game-theoretic data, calls DeepSeek with a
distribution-style tool, and samples the final action from the LLM's
weighted preferences.

The LLM ALWAYS makes the final decision — no auto-play thresholds
override it. The only exceptions are force-switch (fainted Pokemon)
and single-option (one legal move, no switches).
"""
...
from fp.deepseek.tools import build_tools
...
# ---------------------------------------------------------------------------
# Normalization / sampling shared helpers
# ---------------------------------------------------------------------------
...
def _parse_distribution(
    ...
    if not raw:
        logger.warning("No valid weights in DeepSeek distribution, using uniform")
        from fp.deepseek.tools import _build_all_option_strings
...
async def _call_deepseek(
    ...
    """Make a single DeepSeek call and extract the tool call result."""
    if error_context:
        user_prompt += (
    ...
    thinking_budget = getattr(_cfg, "deepseek_thinking_budget", 4096)
    reasoning_effort = getattr(_cfg, "deepseek_reasoning_effort", "high")
    ...
    # DeepSeek silently ignores temperature — only pass it for API compat
    kwargs["temperature"] = temperature

    # Thinking mode (reasoning_effort="high" default)
    ...
    for attempt in range(3):
        try:
            ...
            if any(
                tag in err_str
                for tag in ("429", "500", "503", "overloaded")
            ):
                wait = 2**attempt * 3
                logger.warning(
                    "DeepSeek %s (attempt %d/3), retrying in %ds...",
                    err_str[:80],
                    attempt + 1,
                    wait,
                )
    ...
    if msg.tool_calls:
        ...
        try:
            ...
            args = {}
        logger.info(
            "DeepSeek called %s — distribution entries: %d",
            tc.function.name,
            len(args.get("distribution", [])),
        )
        return {"name": tc.function.name, "args": args}

    resp_text = msg.content or ""
    raise GeminiInvalidChoice(
        f"DeepSeek did not return a tool call. Response: {resp_text[:200]}"
    )
...
async def find_best_move_deepseek(battle) -> list[dict]:
    """Pick the best action(s) using DeepSeek with distribution-based reasoning.

    The LLM receives ALL available data — heuristic scores, threat
    assessment, battle history, strategic context, AND MCTS
    game-theoretic search insights.  It outputs relative preference
    weights (1-10) for each viable option.  The actual action is
    sampled from the normalized distribution.

    The LLM ALWAYS decides.  The only exceptions are forced situations:
    fainted Pokemon (must switch) and a single legal move with no
    switch targets.
    """
    ...
    try:
        ...
        logger.error(
            "DeepSeek decision timed out after %.1fs", _DECISION_TIMEOUT_SECONDS
        )
        raise GeminiTimeout(
            f"DeepSeek call timed out after {_DECISION_TIMEOUT_SECONDS}s"
        )
    except Exception as exc:
        logger.error("DeepSeek decision failed: %s", exc)
        raise
    ...
    if func_name == "choose_leads":
        ...
        logger.info(
            "DeepSeek Distribution (%d options, sharpen=%.1f): sampled=%s, raw=%s",
            len(probs),
            sharpen,
            sampled_option,
