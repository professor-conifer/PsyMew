"""TutorSession — educational coaching mode powered by Gemini.

Provides post-turn commentary, responds to opponent chat, and gives
a post-game review. Uses persistent chat history via the Gemini SDK.
...
class TutorSession:
    """Persistent Gemini chat session for tutoring/coaching during battle."""
    ...
    def _get_client(self):
        if self._client is None:
            ...
            # Flash has no thinking tokens, so max_output_tokens = actual output
            self._model = getattr(FoulPlayConfig, "gemini_tutor_model", "gemini-2.0-flash")
        return self._client

    async def _ask(self, prompt: str, system_context: str = "") -> Optional[str]:
        """Send a prompt to Gemini and return a single sanitized reply string."""
        ...
        config = types.GenerateContentConfig(
            ...
            temperature=0.7,
            # Using gemini-2.0-flash (no thinking tokens), so this = actual output tokens.
            # 200 tokens ~ 150 words, more than enough for 1-2 sentences.
            # _sanitize_reply still hard-caps at 250 chars as a safety net.
        ...
        for attempt in range(2):
            try:
                ...
                reply = _sanitize_reply(response.text or "")
                if reply:
    ...
    async def on_incoming_chat(self, sender: str, text: str) -> Optional[str]:
        ...
        # Ignore self (correctly strip Showdown prefixes like ☆)
